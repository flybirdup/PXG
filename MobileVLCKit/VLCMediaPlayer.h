...
/**
   ...
   * Media must be buffering or playing before it can be recorded.
   *
   * \param recordingFileame name of the file to save the media to, not including any file extension,
   *                     or NULL if vlc should generate the filename automatically
   * \return true if recording was started, false on error
   */
- (BOOL)recordStart:(NSString *)recordingFilePath setFileName:(NSString *)recordingFileName;
...
/**
   ...
   *
   * \return true if recording was stopped, false on error
   */
- (BOOL)recordStop;
